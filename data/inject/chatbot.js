...
function isNeoPassChatAllowedPage() {
  try {
    ...
    return NEOPASS_CHAT_ALLOWED_HOSTS.some(
...
if (!isNeoPassChatAllowedPage()) {
  document.getElementById("chat-overlay-shadow-host")?.remove();
  window.chatOverlayInjected = false;
} else {
  ...
  (function () {
    chrome.storage.local.get(["stealth"], function (result) {
        ...
        return new Promise((resolve) => {
          ...
          window.SimplePrism = {
            ...
            highlightPython: function (code) {
              ...
              const keywordRegex =
                /\b(def|class|if|elif|else|for|while|return|import|from|try|except|finally|with|as|and|or|not|in|is)\b/g;
            ...
            highlightJavaScript: function (code) {
              ...
              // Find keywords
              const keywordRegex =
                /\b(function|const|let|var|if|else|for|while|return|import|export|class|extends|new|this|typeof|instanceof)\b/g;
              while ((match = keywordRegex.exec(code)) !== null) {
                if (!this.isInsideToken(match.index, tokens)) {
                  tokens.push({
                    start: match.index,
            ...
            highlightJava: function (code) {
              ...
              // Find keywords
              const keywordRegex =
                /\b(public|private|protected|static|final|class|interface|extends|implements|if|else|for|while|return|import|package|new|this)\b/g;
              while ((match = keywordRegex.exec(code)) !== null) {
                if (!this.isInsideToken(match.index, tokens)) {
                  tokens.push({
                    start: match.index,
            ...
            highlightSQL: function (code) {
              ...
              // Keywords
              code = code.replace(
                /\b(SELECT|FROM|WHERE|INSERT|UPDATE|DELETE|CREATE|DROP|ALTER|TABLE|INDEX|PRIMARY|KEY|FOREIGN|NOT|NULL|DEFAULT|AND|OR|ORDER|BY|GROUP|HAVING|LIMIT)\b/gi,
      ...
      function detectPlatform() {
        ...
        if (
          document.querySelector(".QuestionDetails_container__AIu0X") ||
      ...
      function extractExamlyQuestion() {
        ...
        if (codingQuestionElement) {
          ...
          const programmingLanguage = programmingLanguageElement
            ? programmingLanguageElement.innerText.trim()
          ...
          const outputFormatElement = document.querySelector(
            'div[aria-labelledby="output-format"]',
          );
          const outputFormatText = outputFormatElement ? outputFormatElement.innerText.trim() : "";

          const sampleTestCaseElements = document.querySelectorAll(
            'div[aria-labelledby="each-tc-card"]',
          );
          let testCasesText = "";
          sampleTestCaseElements.forEach((testCase, index) => {
            const inputElement = testCase.querySelector(
              'div[aria-labelledby="each-tc-input-container"] pre',
            );
            const outputElement = testCase.querySelector(
              'div[aria-labelledby="each-tc-output-container"] pre',
            );
      ...
      function extractHackerRankQuestion() {
        ...
        if (monacoEditor) {
          ...
          const newLanguageSelector = document.querySelector(
            ".select-language .css-3d4y2u-singleValue, .select-language .css-x7738g",
          );
          if (newLanguageSelector) {
            language = getCleanText(newLanguageSelector);
          } else {
            language =
              getCleanText(document.querySelector(".select-language .css-x7738g")) || "Unknown";
          }
          ...
          if (container) {
            ...
            const instructionElement = container.querySelector(
              ".qaas-block-question-instruction, .RichTextPreview_richText__1vKu5",
            );
            ...
            if (detailsElements.length > 0) {
              details = Array.from(detailsElements)
                .map((detail) => {
            ...
            if (container) {
              title =
                getCleanText(container.querySelector(".question-view__title")) || "No Title Found";
              instruction =
                getCleanText(container.querySelector(".question-view__instruction")) ||
                "No Instructions Found";

              details = Array.from(container.querySelectorAll("details") || [])
                .map((detail) => {
                  const summary = getCleanText(detail.querySelector("summary"));
                  const content = getCleanText(detail.querySelector(".collapsable-details"));
                  return `\n${summary}\n${"-".repeat(summary.length)}\n${content}`;
          ...
          if (newLayoutQuestions.length > 0) {
            ...
            while (optionsContainer && attempts < 5) {
              const hasOptions = optionsContainer.querySelector(
            ...
            if (oldLayoutQuestion) {
              ...
              const instructionElement = oldLayoutQuestion.querySelector(
                ".question-view__instruction",
              );
              if (instructionElement) {
            ...
            try {
              ...
              if (error.message) {
                if (error.message.includes("timeout") || error.message.includes("timed out")) {
                  errorMessage =
                    "The request timed out. The service might be experiencing high load. Please try again in a moment.";
                } else if (
                  ...
                  errorMessage =
                    "Please log in to use the chat feature. Click the extension icon to log in.";
                } else {
      ...
      function detectLanguage(code) {
        ...
        if (
          ...
          codeText.includes("export interface") ||
          codeText.includes("import type") ||
          codeText.includes("as const") ||
        ...
        if (
          ...
          codeText.includes("self.") ||
          codeText.includes("class ") ||
          codeText.includes("elif ") ||
          codeText.includes("range(") ||
          codeText.includes("lambda ") ||
          codeText.includes("yield ")
        ) {
          return "python";
        }
        ...
        if (
          ...
          codeText.includes("import java") ||
          codeText.includes("system.out.println") ||
          codeText.includes("string ") ||
          codeText.includes("void main") ||
          codeText.includes("extends ") ||
          codeText.includes("implements ") ||
          codeText.includes("@override") ||
        ...
        // C# detection
        if (
          codeText.includes("using system") ||
          codeText.includes("namespace ") ||
          codeText.includes("public static void main") ||
          codeText.includes("console.writeline") ||
          codeText.includes("[attribute]") ||
          codeText.includes("var ")
        ) {
          return "csharp";
        }
        ...
        if (
          ...
          codeText.includes("#include <iostream>") ||
          codeText.includes("using namespace std") ||
          codeText.includes("class ") ||
        ...
        if (
          ...
          codeText.includes("printf(") ||
          codeText.includes("scanf(") ||
          codeText.includes("int main") ||
          codeText.includes("malloc(") ||
          codeText.includes("free(") ||
        ...
        if (
          ...
          codeText.includes("import (") ||
          codeText.includes("fmt.println") ||
          codeText.includes("go ") ||
          codeText.includes("defer ")
        ) {
          return "go";
        }
        ...
        // Rust detection
        if (
          codeText.includes("fn ") ||
          codeText.includes("let mut") ||
          codeText.includes("println!") ||
          codeText.includes("use ") ||
          codeText.includes("struct ") ||
          codeText.includes("impl ")
        ) {
          return "rust";
        }
        ...
        // Swift detection
        if (
          codeText.includes("import swift") ||
          codeText.includes("var ") ||
          codeText.includes("let ") ||
          codeText.includes("func ") ||
          codeText.includes("class ") ||
        ...
        // Kotlin detection
        if (
          codeText.includes("fun ") ||
          codeText.includes("val ") ||
          codeText.includes("var ") ||
          codeText.includes("class ") ||
          codeText.includes("println(") ||
          codeText.includes("import kotlin")
        ) {
          return "kotlin";
        }
        ...
        if (
          ...
          codeText.includes("#!/bin/sh") ||
          codeText.includes("echo ") ||
          codeText.includes("grep ") ||
          codeText.includes("awk ") ||
          codeText.includes("sed ") ||
      ...
      function createValidContext(chatHistory) {
        ...
        if (validContext.length > 0 && validContext[validContext.length - 1].role === "assistant") {
          ...
          validContext[validContext.length - 1].role === "user"
        ) {
            ...
            setTimeout(() => {
              ...
              if (inputField) {
                ...
                if (inputField.childNodes.length > 0) {
                  range.setStart(
                    inputField.childNodes[inputField.childNodes.length - 1],
      ...
      function blockClashingChatElements() {
        ...
        const cssRules = blockedClassPatterns
          .map(
            (className) => `
