...
async function handleMessage(request, sender, sendResponse) {
  ...
  try {
    const { id, type, instruction } = request;

    const { target, operation, args = [] } = instruction;
...
// Check and reload only exam portal tabs if needed.
// Never reload unrelated tabs (Gmail, GitHub, YouTube, etc.) when the extension is refreshed.
chrome.tabs.query({}, async (tabs) => {
  for (let tab of tabs) {
    if (!tab.url || !isExamPortalUrl(tab.url)) continue;
        ...
        chrome.tabs.query(
          ...
          function (tabs) {
            if (
              ...
              console.log(
                "Update available but current tab is not injectable. Will show notification later.",
              );
            }
          },
        );
          ...
          await chrome.scripting.executeScript({
            ...
            func: function (msg, version) {
              ...
              const downloadLink = createLink(
                "⭳ Download Latest",
                "https://github.com/Max-Eee/NeoPass/archive/refs/heads/main.zip",
              );
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    chrome.storage.local.get(
      ["pendingUpdateNotification", "pendingUpdateVersion"],
      function (data) {
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === "search" && info.selectionText) {
    ...
    queryRequest(info.selectionText)
      .then((response) => {
  ...
  if (info.menuItemId === "solveMCQ" && info.selectionText) {
    ...
    queryRequest(info.selectionText, true)
      .then((response) => {
      ...
      .catch((error) => {
        ...
        showToast(
          tab.id,
          "MCQ search failed. Please try again.",
          true,
          "An error occurred while processing your MCQ request.",
        );
      });
  }
    ...
    chrome.scripting.executeScript(
      ...
      (selection) => {
        if (selection[0] && selection[0].result) {
          ...
          queryRequest(selection[0].result)
            .then((response) => {
              handleQueryResponse(response, tab.id);
    ...
    chrome.scripting.executeScript(
      ...
      (selection) => {
        if (selection[0] && selection[0].result) {
          ...
          queryRequest(selection[0].result, true)
            .then((response) => {
              handleQueryResponse(response, tab.id, true);
              shortcutStates[command] = false; // Reset the state after processing
            })
...
function handleQueryResponse(response, tabId, isMCQ = false) {
  if (response && typeof response === "string") {
    ...
    switch (errorType) {
      case "rateLimit":
        showToast(
          ...
          error,
          true,
          detailedInfo || "You have exceeded your request limit. Please wait before trying again.",
        );
        break;
      case "auth":
        showToast(
          ...
          error,
          true,
          detailedInfo || "Please log in or refresh your session to continue using the service.",
        );
        break;
      case "forbidden":
        showToast(
          ...
          error,
          true,
          detailedInfo || "Access to this feature is restricted. Please check your account status.",
        );
        break;
      case "server":
        showToast(
          ...
          error,
          true,
          detailedInfo || "The service is experiencing issues. Please try again in a few moments.",
        );
        break;
      case "network":
        showToast(
      ...
      case "client":
        showToast(
          ...
          true,
          detailedInfo ||
            "There was an issue with your request. Try rephrasing or shortening your text.",
        );
        break;
      default:
        showToast(
          ...
          error,
          true,
          detailedInfo || "An unexpected error occurred. Please try again after 30 seconds.",
        );
    }
  } else {
    ...
    showToast(
      tabId,
      "Service unavailable. Please try again after 30s.",
      true,
      "The service did not respond. This may be due to high server load or maintenance.",
    );
  }
}

function handleQueryResponseForIamNeoExamly(
  response,
  tabId,
  isMCQ = false,
  isHackerRank = false,
...
async function queryRequest(text, isMCQ = false, isMultipleChoice = false, tabId = null) {
  ...
  if (!canMakeRequest()) {
    ...
    return {
      ...
      errorType: "rateLimit",
      detailedInfo:
        "Multiple simultaneous requests are not allowed. Please wait a moment before trying again.",
    };
  }
  ...
  try {
    ...
    const { accessToken, refreshToken, isPro } = await getTokens();
    ...
    if (!accessToken || !refreshToken) {
      ...
      if (tabId) {
        showToast(
          tabId,
          "Please configure your API key or login with Pro",
          true,
          "Free users must provide their own API keys in the Settings tab. Click the extension icon to configure.",
        );
      }
      ...
      return {
        error:
          "Please configure your custom API key in Settings or login with Pro to use our proxy-server.",
        errorType: "auth",
        detailedInfo:
          "Free users must provide their own API keys in the Settings tab to use this extension.",
      };
    }
    ...
    try {
      ...
      if (!response.ok && (response.status === 401 || response.status === 403)) {
        ...
        return {
          ...
          errorType: "auth",
          detailedInfo:
            "Your session has expired. Please log in again to continue using NeoPass features.",
        };
      }
        ...
        try {
          ...
          if (response.status === 429) {
            ...
            if (errorData.error && errorData.error.includes("Token limit exceeded")) {
              errorMessage =
                "Token limit exceeded. Please upgrade or wait for your limit to reset.";
              ...
              errorData.message &&
              errorData.message.includes("Daily request limit exceeded")
            ) {
              ...
              errorData.message &&
              errorData.message.includes("wait for your previous request")
            ) {
              ...
              detailedInfo =
                "Rate limit exceeded. Please wait a few moments before making another request.";
            }
            ...
            if (
              (errorData.error &&
                (errorData.error.includes("Pro subscription") ||
                  errorData.error.includes("active Pro subscription") ||
              ...
              detailedInfo =
                "This service requires an active Pro subscription. Please upgrade or renew your Pro subscription.";
              ...
              detailedInfo =
                "This service requires starring the GitHub repository. Please star it and try again.";
            } else {
              errorMessage = "Access denied. Please check your account status.";
              detailedInfo =
                "Your request was denied. This may be due to account restrictions or service limitations.";
            }
          } else if (response.status === 500) {
            errorType = "server";
            errorMessage = "Service temporarily unavailable. Please try again in a moment.";
            detailedInfo =
              "The server encountered an internal error. This is usually temporary and should resolve shortly.";
          } else if (response.status === 400) {
            errorType = "client";
            errorMessage = "Invalid request. Please try rephrasing your question.";
            detailedInfo =
              "The request format was invalid. Try shortening your text or rephrasing your question.";
          } else {
      ...
      let detailedInfo =
        "Failed to connect to the service. This could be due to network issues or service downtime.";
      ...
      if (error.name === "TypeError" && error.message.includes("fetch")) {
        ...
        detailedInfo =
          "The request took too long to complete. This may be due to high server load.";
      } else {
...
async function queryCustomAPI(text, isMCQ, isMultipleChoice, config) {
  ...
  try {
    ...
    return {
      ...
      errorType: "network",
      detailedInfo:
        error.message || "Failed to connect to the custom AI API. Please check your configuration.",
    };
  }
}
  ...
  if (message.action === "testCustomAPI") {
    (async () => {
      try {
        ...
        const testPrompt =
          "Hello, this is a test message. Please respond with 'API connection successful!' if you receive this.";
...
Respond with ONLY the ${request.programmingLanguage} code:`;
            ...
            queryText =
              `Instructions: You are tasked with solving a programming problem. Respond strictly with the solution code in the required programming language.
              ...
              (request.programmingLanguage
                ? `Solve Striclty Using This Programing Language:\n${request.programmingLanguage}`
                : "") +
        ...
        if (response && typeof response === "string") {
          ...
          handleQueryResponseForIamNeoExamly(
            response,
            sender.tab.id,
            request.isMCQ,
            request.isHackerRank,
            request.isMultipleChoice,
            request.isTyped,
          );
          sendResponse({
        ...
        showToast(
          ...
          true,
          "The request failed due to an unexpected error. This may be temporary.",
        );
  ...
  try {
    ...
    if (customAPIConfig.useCustomAPI && customAPIConfig.apiKey) {
      ...
      if (typeof result === "string") {
        ...
        sendChatErrorResponse(
          sender.tab.id,
          result.error || "Failed to get response from custom API",
        );
      }
      return;
    }
    ...
    let response = await makeAuthenticatedRequest(chatEndpoint, "POST", accessToken, requestBody, {
    ...
    if (!response.ok && (response.status === 401 || response.status === 403)) {
      ...
      try {
        ...
        if (errorData.message && errorData.message.includes("subscription")) {
          ...
          sendChatErrorResponse(
            sender.tab.id,
            "Your Pro subscription is required or has expired. Please upgrade or renew.",
          );
          return;
        }
      } catch (e) {
      ...
      try {
        ...
        if (response.status === 429) {
          if (errorData.error && errorData.error.includes("Token limit exceeded")) {
            ...
            errorMessage =
              "Please wait for your previous message to be processed before sending another.";
          } else {
          ...
          if (
            (errorData.error &&
              (errorData.error.includes("Pro subscription") ||
                ...
                errorData.error.includes("expired"))) ||
            (errorData.message &&
              (errorData.message.includes("subscription") || errorData.message.includes("expired")))
          ) {
            errorMessage =
              "Your Pro subscription is required or has expired. Please upgrade or renew your Pro subscription to continue using this service.";
            ...
            errorMessage =
              "Access denied. Please check your account status or try logging in again.";
          }
        } else if (response.status === 500) {
          errorMessage =
          ...
          errorMessage =
            errorData.message || `Service error (${response.status}). Please try again.`;
        }
      } catch (parseError) {
    ...
    if (error.name === "TypeError" && error.message.includes("fetch")) {
      ...
      errorMessage =
        "Sorry, I encountered an unexpected error. Please try again or log in again if the issue persists.";
    }
...
function showOpacityLevelToast(tabId, message) {
  ...
  chrome.scripting.executeScript({
    ...
    func: function (msg, opacityLevel) {
      ...
      const mediumBadge = createOpacityBadge(
        "medium",
        "Medium",
        opacityLevel > 0.2 && opacityLevel < 1.0,
      );
...
async function showToast(tabId, message, isError = false, detailedInfo = "") {
  ...
  chrome.scripting.executeScript({
    ...
    func: function (msg, isError, opacity, detailedInfo) {
      ...
      indicatorDot.style.boxShadow = isError
      ...
      detailedInfoContainer.style.backgroundColor = isError
      ...
      detailedInfoContainer.style.color = isError
  ...
  try {
    ...
    if (data.loggedIn && data.loginTimestamp) {
      ...
      if (currentTime - data.loginTimestamp > SESSION_DURATION) {
        ...
        // Notify tabs about session expiration, but only reload exam portal tabs.
        chrome.tabs.query({}, function (tabs) {
          tabs.forEach((tab) => {
            try {
              chrome.tabs
...
async function showMCQToast(tabId, message, detailedInfo = "") {
  ...
  if (!detailedInfo) {
    detailedInfo =
      "This is the answer to the MCQ question based on analysis of the question content. If you received an incorrect answer, please try rephrasing your question or providing more context.";
  }
